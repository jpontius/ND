from icalendar import Calendar, Event
import shutil
import requests
import arrow

#################################################################
# Get resource ics file from reservaiton system
url = 'http://192.168.192.47/booked/Web/export/ical-subscribe.php?uid=&sid=&rid=58ecd28916533&icskey=nmr'
response = requests.get(url, stream=True) # dont verify ssl certs as I'm using self signed.
with open('/tmp/400.ics', 'wb') as out_file:
    shutil.copyfileobj(response.raw, out_file)
del response
#################################################################

now = arrow.now()# get time now
now_text = arrow.now().format('HH:mm MM-DD-YYYY')
g = open('/tmp/400.ics','rb')# open ics file for reading
gcal = Calendar.from_ical(g.read())#read ics file
#print "starting"
file400 = open("/home/pi/reservations/400.txt", "w")# open reservations txt file for write
file400.write('Bruker 400 - Upcoming Reservations'+'\n') #write
file400.write('Last update: '+now_text +'\n')#write
for component in gcal.walk():# loop through all events
    if component.name == "VEVENT":# loop through all events
        Summary = component.get('summary')# get summary
        end = component.get('dtend') # get reservation end time
        end_time = arrow.get(end.dt).replace(tzinfo='UTC')# arrow object add tzinfo
        if end_time > now: # if ending time is later than now
            file400.write(Summary+'\n')# then write info to txt file.
            #print component.get('summary')
g.close()# close ics file
file400.close()# close txt file
